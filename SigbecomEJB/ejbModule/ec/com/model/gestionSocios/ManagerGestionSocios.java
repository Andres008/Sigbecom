...
public class ManagerGestionSocios {
	...
	public UsrInstitucionBancaria buscarInsitucionBancariaById(long idInstitucionBancaria) throws Exception {
		try {
			return (UsrInstitucionBancaria) managerDAOSegbecom.findById(UsrInstitucionBancaria.class,
					idInstitucionBancaria);
		} catch (Exception e) {
	...
	public List<UsrCanton> buscarCantoByProvincia(String idProvincia) throws Exception {
		try {
			return managerDAOSegbecom.findWhere(UsrCanton.class,
					"o.idCanton <> '0' and o.usrProvincia.idProvincia='" + idProvincia + "'", "o.canton ASC");
		} catch (Exception e) {
	...
	public List<UsrParroquia> buscarParroquiaByCanton(String idCanton) throws Exception {
		try {
			return managerDAOSegbecom.findWhere(UsrParroquia.class,
					"o.idParroquia <> '0' and o.usrCanton.idCanton='" + idCanton + "'", "o.parroquia ASC");
		} catch (Exception e) {
	...
	@SuppressWarnings("unchecked")
	public List<UsrTipoSocio> buscarTodoTipoSocio() throws Exception {
		try {
			List<UsrTipoSocio> result = managerDAOSegbecom.findAll(UsrTipoSocio.class, "o.estado ASC");
			result.forEach(descuento -> descuento.getUsrTipoDescuentoSocios()
					.forEach(tipoDes -> tipoDes.getUsrTipoDescuento().getNombre()));
			return result;
		} catch (Exception e) {
			throw new Exception("Atención, error al cargar tipo de Socios, comuniquise con el administrador.");
		}
	}

	public UsrTipoSocio buscarTipoSocioById(long idTipoSocio) throws Exception {
		try {
			UsrTipoSocio tipoSocio = (UsrTipoSocio) managerDAOSegbecom.findById(UsrTipoSocio.class, idTipoSocio);
			tipoSocio.getUsrTipoDescuentoSocios().forEach(descuentos -> descuentos.getUsrTipoDescuento().getNombre());
			return tipoSocio;
		} catch (Exception e) {
